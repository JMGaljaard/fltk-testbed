...
def get_sampler(dataset, args):
    ...
    if args.get_distributed():
        ...
        if method == "uniform":
            sampler = DistributedSampler(dataset, rank=args.get_rank(), num_replicas=args.get_world_size())
        elif method == "q sampler":
            sampler = Probability_q_Sampler(dataset, rank=args.get_rank(), world_size=args.get_world_size(),
                                            *args.get_sampler_args())
        elif method == "limit labels":
            sampler = LimitLabelsSampler(dataset, rank=args.get_rank(), world_size=args.get_world_size(),
                                         *args.get_sampler_args())
        elif method == "dirichlet":
            sampler = DirichletSampler(dataset, rank=args.get_rank(), world_size=args.get_world_size(),
                                       *args.get_sampler_args())
        else:  # default
